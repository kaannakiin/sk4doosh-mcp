# Why sk-mcp is not an OpenAPI adapter
...
integration is the best-known example. sk-mcp starts from a different place, and the difference is
...
sk-mcp sits **inside** it. An SDK reads the framework's own route and authorization metadata,
...
chain atomic calls badly, and near-duplicate tools make them hesitate. sk-mcp's answer is structural
...
## What sk-mcp refuses to do silently

Every adapter we reviewed resolves something quietly. These are the resolutions sk-mcp turns into
errors or declines outright:

| An adapter may…                                          | sk-mcp instead…                                                              |
| -------------------------------------------------------- | ---------------------------------------------------------------------------- |
...
Embedding needs an SDK, and sk-mcp has two: ASP.NET Core and NestJS. A backend on any other stack
can still be reached through its OpenAPI document, and sk-mcp reads Swagger 2.0 and OpenAPI 3.0–3.2
...
pass. A behaviour that only one implementation has is not a behaviour sk-mcp promises.
