...
- **Sentetik istek kendini tanıtır (M9, M10):** istek bağlamında bir işaret taşınır (C#: `HttpContext.Items`, `IsSkMcpRequest()`), probe bayrağının kardeşi. Host tarayıcıya özgü dönüşümlerini (gövde şifreleme, compression, oturum dokunuşu, erişim logu, rate limit) bu işaretle atlayabilir. **`UserAgent` bu iş için kullanılamaz:** dış istek onu taklit edebilir, o yüzden gözlemlenebilirlik sinyalidir, güvenlik dalı değil. İşaret dışarıdan set edilemez (M9). M10 dalın gerçekten yettiğini kanıtlar: zorunlu gövde şifrelemesi + yanıt dönüşümü kurulu bir host'ta tarayıcı isteği şifreli yanıt alıp şifresiz gövdeyi reddederken, aynı pipeline'dan geçen sentetik istek düz JSON alıp düz gövde gönderir.
