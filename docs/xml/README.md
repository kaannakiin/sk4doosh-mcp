...
Tarih: 2026-09-09. Durum: **F0 kapısı geçti; F1'in XML kapıları uygulandı ve `packages/xml-mcp` var. F2 kısmen tamamlandı (F2-01/02/03).**

Güncel kaynak/doküman tabanı `6b2bc89`; [CI #34226587889](https://github.com/kaannakiin/sk4doosh-mcp/actions/runs/34226587889) 13/13 başarılı. Beş native hedef × Node 22/24, gerçek MCP ve birleşik paket doğrulaması tamamlandı. Son kod düzeltmesi `4472332` ve bulgu bazlı kanıtlar [kapanış kaydında](excel-hardening-uygulama.md). `@sk-mcp/xml-mcp` `0.1.0` repoda ve `list_documents` gerçek MCP istemcisiyle çalışıyor; F1'in XML kapılarının ölçümleri [F1 kapanış kaydında](xml-f1-kapanis.md). Kalan üç tool F2-04–09'dadır.
...
| [F1 — Ortak çekirdek ve Excel](fazlar/01-ortak-cekirdek-ve-excel.md) | Dosya güvenliği, Excel düzeltmeleri ve XML kaynak ömrü | XML yaşam döngüsü için F0           | F1-03/05/06 uygulandı; platform CI bekleniyor         |
| [F2 — Salt okunur MVP](fazlar/02-okuma-mvp.md)                       | Dört tool ile keşif, düğüm okuma ve metin arama        | F0 + F1 (karşılandı)                | Kısmen tamamlandı; F2-01/02/03 kapandı                |
...
Excel/file-core regresyonları, F0 motor deneyleri ve F1'in XML kapıları tamamlandı. `packages/xml-lab` F0 harness'ı olarak kalıcıdır; ürün kodu `packages/xml-mcp`'dedir ve ikisi hiçbir yönde birbirini import etmez. Sıradaki iş F2-04 genişletilmiş düğüm modelidir.
