...
Durum: **kısmen tamamlandı** (2026-09-09). F2-01/02/03 uygulandı ve F1'in XML kapılarını kapattı; kanıt [F1 kapanış kaydında](../xml-f1-kapanis.md). F2-04–09 açık. Sorumlu: XML geliştiricisi; doğrulayan: entegrasyon/test inceleyicisi. Önkoşul karşılandı: F0 geçti, F1'in XML kapıları uygulandı.

## Hazır altyapı ve kalan entegrasyon

F1-01/02/04/07'nin ortak dosya erişimi, snapshot/cache ve listeleme uygulaması platform CI'ında geçti. F2 bu altyapıyı tüketiyor: parser girdisi `ParseContext.bytes`, içerik kimliği `stamp`, kullanıcıya gösterilen yol `displayPath`; `path` parser tarafından yeniden açılmıyor. `packages/xml-mcp`, XML format registry'si, tek parse policy'si, worker yürütücüsü ve `list_documents` F2-01/02/03 ile teslim edildi. Genişletilmiş node modeli ve kalan üç tool henüz yok; F2-04–09 açık. Ortak kaynak güvenliğinin XML handler'ına doğru bağlandığı F1-06 bağlantı testleriyle kanıtlandı.
...
F2-01/02/03, F1'in kapanış kapısı olduğu için erken tamamlandı. Görev kimlikleri değişmedi; kanıtları [F1 kapanış kaydındadır](../xml-f1-kapanis.md).
...
F2-01/02/03 kapandı. Kalan F2-04–09 test/inceleme kanıtıyla tamamlanacak; [test matrisinin](../test-stratejisi.md) F2 zorunlu aileleri geçecek; nihai limitler — özellikle sayfa ve yanıt zarfı bütçesi — ölçülecek; ardından F6 kurulum/yayın kapısına hazır olunur. Benchmark veya güvenlik testi “sonra” bırakılarak MVP tamamlandı denmez.
