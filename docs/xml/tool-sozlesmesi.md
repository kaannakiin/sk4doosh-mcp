...
Bu belge XML tool sözleşmesidir. Güncelleme: 2026-09-09. `packages/xml-mcp` var ve `list_documents` bu belgedeki davranışı uyguluyor; `describe_document`, `read_node` ve `find_in_document` hâlâ hedef davranıştır ve F2-05/06/07'de şemaya dönüşecek. Uygulanmış satırlar tabloda işaretlidir. Başlangıç bütçeleri F0-08 ile ölçülüp [türetildi](f0-kanit-kaydi.md); yanıt zarfı kapısı F1 kapanışında `file-core`'a girdi, sayfa bütçesi hâlâ F2-08'e aittir.
...
`totalMatches` yalnız tam taramada kullanılır; erken durmada `scannedCount` ve `matchedSoFar` döner. Mevcut ortak hata kodları `file_changed`, `unsupported_platform` ve `resource_limit` dahil yeniden kullanılır; aynı durum için ayrıca `document_changed` kodu türetilmez. XML'e özgü kodlardan `malformed_xml` ve `doctype_not_allowed` F2-02 ile kesinleşti ve yayınlanan `SkMcpXmlErrorCode` birleşiminde duruyor. `stale_cursor` ve `invalid_cursor` zaten ortak kodlardır, ayrıca türetilmedi; `query_timeout` yerine `resource_limit` kullanılır. `unsupported_encoding` bu teslimde **ilan edilmedi**: motor, encoding ve well-formedness hatalarını aynı istisnayla bildiriyor ve ayrım motor-mesajı eşlemesi gerektiriyor (F2-02). `query_not_supported` F3'e aittir. Başarısız parse kısmi başarılı belge üretmez.
