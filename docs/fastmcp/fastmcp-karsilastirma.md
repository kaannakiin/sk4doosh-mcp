...
**Durum:** Tasarım notu. §4.1–§4.9 sevk edildi (§7–§15); notun açık maddesi kalmadı. Karar kayıtları: [govde-tipleri-karari.md](govde-tipleri-karari.md), [arguman-kuratorlugu-karari.md](arguman-kuratorlugu-karari.md), [outputschema-karari.md](outputschema-karari.md), [invoke-korumalari-karari.md](invoke-korumalari-karari.md), [search-detail-karari.md](search-detail-karari.md), [selection-rule-karari.md](selection-rule-karari.md), [deepobject-karari.md](deepobject-karari.md)
**Tarih:** 17 Eylül 2026
...
**Kapsam:** `packages/http/spec`, `packages/http/core`, `sdks/*` — HTTP katalog ürün hattı. `file-core` ve `products/chat` bu notun dışında
...
- **Çağrı:** `RequestDirector.build` düz argümanları `parameter_map` ile konumlarına dağıtır. **Bilinmeyen argüman log warning ile sessizce düşer.** Query array style/explode OpenAPI default'larıyla; path array virgülle; `deepObject` destekli; cookie parametreleri destekli. Body: JSON, `+json`, `text/plain`, `x-www-form-urlencoded`, `multipart/form-data`. Dosya parçası yalnız FastMCP Python'dan çağrıldığında oluşur; MCP üzerinden gelen bir "dosya" dosya adı olmayan bir metin alanıdır ([govde-tipleri-karari.md](govde-tipleri-karari.md)). MCP isteğinin HTTP header'ları, backend isteğinde zaten yoksa **hepsi** iletilir (`get_http_headers()`).
...
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **İsim çakışması**                | Sessiz `_2`; 56 karakterde sessiz kesme; `operationId` `__`'ye kadar                                                                                | `name_collision` fatal; 256 sınır + `long_tool_name` uyarısı; container prefix + tekrar bastırma; çoklu route → tek tool + `alternateRoutes` ([naming.md](../../packages/http/spec/naming.md))                                                   | **Biz.** Sessiz `_2`, komşu endpoint eklendiğinde mevcut tool'un adını kaydırır; ajanın öğrendiği isim kararsızlaşır                                        |
| **Bilinmeyen argüman**            | Log warning, argüman sessizce düşer                                                                                                                 | `unknown_argument` + izinli isim listesi ([argument-mapping.md](../../packages/http/spec/argument-mapping.md) adım 1)                                                                                                                            | **Biz**                                                                                                                                                     |
| **Parametre/body isim çakışması** | Parametre `id__path` olur, description'a "(Path parameter)" eklenir, body düz kalır                                                                 | Body `body` kök argümanına iner, `body_field_collision` uyarısı ([schema-conversion-rules.md](../../packages/http/spec/schema-conversion-rules.md) Tablo 6)                                                                                      | **Berabere, tartışılır** (§5)                                                                                                                               |
...
| **Array serileştirme**            | style/explode OpenAPI default'ları; path array virgül; `deepObject`; cookie                                                                         | Aynı default'lar; delimiter encoder'dan geçmez (iki dilin encoder farkı); boş array anahtar yazmaz; path array yasak; `spaceDelimited+explode:true` reddedilir                                                                                   | Parite. `deepObject` §14'te geldi; cookie bilinçli olarak yok (§5)                                                                                          |
| **Path encoding**                 | `httpx`'e bırakılır                                                                                                                                 | Katı RFC 3986, `!'()*` dahil; ham birleştirme yasak (`5/../admin` tek segment)                                                                                                                                                                   | **Biz**; fixture-pinned                                                                                                                                     |
| **Body tipleri**                  | JSON, `+json`, `text/plain`, form-urlencoded, multipart (dosya)                                                                                     | Sadece JSON; form/file → `unsupported_binding`, endpoint düşer                                                                                                                                                                                   | **FastMCP daha geniş** (§4.8), ama dosya parçası MCP üzerinden oluşmuyor                                                                                    |
| **Hata haritalama**               | Tam body, 401 dahil, ajana akar; sızıntı filtresi yok                                                                                               | Dokuz kodlu sözlük, `retryable`, `fields`, `reference`; 401 body asla, 5xx body asla, HTML asla; pattern deny-list; host recognizer ([error-mapping.md](../../packages/http/spec/error-mapping.md))                                              | **Biz, açık ara**                                                                                                                                           |
| **Output schema**                 | 2xx şeması `outputSchema`; `{result}` sarma; `structured_content`; `validate_output`                                                                | `load_tool` `outputSchema` döner; birincil response sırası 200/201/202/204, object olmayan kök `{result}` ile sarılır, `$defs` sarmalayıcı köke kalkar, readOnly korunur ([metadata-contract.md](../../packages/http/spec/metadata-contract.md)) | **Berabere** (§8). `structuredContent` doğrulaması bilinçli kapsam dışı: `invoke_tool` sabit meta-tool, şeması çağrı başına değişir                         |
...
| **Search algoritması**            | BM25 k1=1.5; NFKC casefold; prefix yok; alan ağırlığı yok; **parametre adları/açıklamaları indekste**                                               | BM25 k1=1.2; alan ağırlıkları; prefix eşleşme; NFD folding; sondaki `s`; fixture-pinned ([search-semantics.md](../../packages/http/spec/search-semantics.md))                                                                                    | Bizimki Türkçe için özellikle daha iyi. Parametre adları ve açıklamaları artık bizde de indekste (§10), ama küratörlenmiş yüzeyden ve yalnız kök derinlikte |
| **Görünürlük**                    | enable/disable; session bazlı; allowlist; component `auth` hem gizler hem reddeder                                                                  | Üç değerli karar; T0–T2 ladder; probe; `authUncertain`; caller-scope cache; invariant 1: invoke asla visibility'ye bakmaz ([visibility.md](../../packages/http/spec/visibility.md))                                                              | **Biz.** Onlarınki MCP token'ına bağlı, backend yetkisinden habersiz. Bizde karşılığı olmayan tek şey session-scoped progressive disclosure (§5)            |
| **Seçim**                         | Sıralı `RouteMap` (regex/tag), default her şey TOOL, ilk eşleşen kazanır                                                                            | Dört seviye most-specific-wins; global default `exclude`; config kuralları sırasız, çelişki `ambiguous_selection` fatal ([selection-hierarchy.md](../../packages/http/spec/selection-hierarchy.md))                                              | **Biz.** Bizimki default-deny, ve config kuralı §13'te geldi — ama sıralı liste olarak değil: "ilk eşleşen kazanır" sessiz çözümdür (§4.6, §13)             |
| **Transport / auth**              | OAuth server, OAuth/OIDC proxy, token verifier, `AuthMiddleware`, `InsufficientScopeError`                                                          | PRM + 401 decoration + audience kuralı; enforcement host'a bırakılır ([transport.md](../../packages/http/spec/transport.md))                                                                                                                     | Farklı roller: onlar auth **sağlar**, biz host'un auth'unu **kullanırız**. Tutarlı                                                                          |
...
- Spec: [metadata-contract.md](../../packages/http/spec/metadata-contract.md) tool definition tablosu, [search-semantics.md](../../packages/http/spec/search-semantics.md) meta-tool kontratı, `tool-definition.schema.json`. Fixture: `metadata-extraction` altına 3–4 vaka (object kök, array kök, 204 boş, 2xx yok).

### 4.2 Invoke payload bütçesi ve timeout — **sevk edildi (§9)**

Karar kaydı: [invoke-korumalari-karari.md](invoke-korumalari-karari.md). Spec: [invoke-semantics.md](../../packages/http/spec/invoke-semantics.md).
...
- Spec: [search-semantics.md](../../packages/http/spec/search-semantics.md) alan tablosu. Fixture: `search` altına 2–3 vaka; mevcut fixture'ların skorları değişebilir, aynı değişiklikte güncellenir.
...
### 4.8 Form ve multipart body — **sevk edildi (§15)**
...
### 4.9 `deepObject` query style ve cookie parametreleri — **sevk edildi (§14)**
...
| 6    | §4.8 form body **(tamamlandı, §15)**                                                            | request-bodies, schema-conversion-rules Tablo 7, argument-mapping | argument-mapping, metadata-extraction, error-mapping |

Adım 1 ve 2 küçük spec değişikliği + fixture ile kapanır. **Adım 5 sıradan çıktı ve önce sevk edildi** — gerekçesi ve sonucu §7'de. **Adım 1 iki yarısıyla da kapandı: `outputSchema` §8'de, payload bütçesi ve timeout §9'da; adım 2 §10'da, adım 3'ün `detail` yarısı §11'de.** Adım 3 §12 ile tümüyle kapandı, adım 4 §13 ile; §4.9 §14 ile, adım 6 §15 ile. **Notun açık maddesi kalmadı.**
...
- **Normatif spec:** [packages/http/spec/argument-curation.md](../../packages/http/spec/argument-curation.md). `endpoint-descriptor.schema.json` `arguments` ve `variants` alanlarını kazandı; `fixture.schema.json` dört mevcut kind'ın girdisini genişletti. Üretilmiş tipler iki dilde yenilendi.
- **Kural katmanı:** tek çözümleyici iki tüketiciye veriyor — `packages/http/core/src/curation.ts` ve `SkMcp.AspNetCore/Curation.cs`. Şablon iki ad taşıyor (tel + ajan), `compose` üçüncü bir saf parametre (`deferred`) alıyor, deny-list serbest biçimli gövdede bile geçerli, doldurulan değerler ajanın gönderdiğiyle aynı tip kapısından ve aynı kodlamadan geçiyor.
...
§4.1 sevk edildi. Karar kaydı [outputschema-karari.md](outputschema-karari.md), normatif spec [packages/http/spec/metadata-contract.md](../../packages/http/spec/metadata-contract.md) `## Producing outputSchema`.
...
§4.2 sevk edildi. Karar kaydı [invoke-korumalari-karari.md](invoke-korumalari-karari.md), normatif spec [packages/http/spec/invoke-semantics.md](../../packages/http/spec/invoke-semantics.md) — composition ile backend yanıtı arasını tarif eden ilk doküman.
...
§4.3 sevk edildi. Normatif spec [packages/http/spec/search-semantics.md](../../packages/http/spec/search-semantics.md); yeni karar kaydı yok, çünkü karar notun kendisindeydi ve kapsam küçüktü.
...
§4.5 sevk edildi. Karar kaydı [search-detail-karari.md](search-detail-karari.md), normatif spec [packages/http/spec/search-semantics.md](../../packages/http/spec/search-semantics.md) `## The loaded shape`.
...
§4.7 sevk edildi. Yeni karar kaydı yok; kararlar notun kendisindeydi, genişleyen iki madde aşağıda gerekçesiyle yazılı. Normatif spec [packages/http/spec/search-semantics.md](../../packages/http/spec/search-semantics.md) `## Filtering by tag`.
...
§4.6 sevk edildi. Karar kaydı [selection-rule-karari.md](selection-rule-karari.md), normatif spec [packages/http/spec/selection-hierarchy.md](../../packages/http/spec/selection-hierarchy.md) `## Config-level rules`.
...
## 15. §4.8 sevkiyatı — JSON dışı gövdeler ve dosya argümanı

Karar kaydı: [govde-tipleri-karari.md](govde-tipleri-karari.md). Normatif spec: [packages/http/spec/request-bodies.md](../../packages/http/spec/request-bodies.md).

**Ne sevk edildi.** Descriptor'ın `requestBody`'si tek bir somut `contentType` taşıyor; yoksa `application/json`. JSON ailesi (`*+json`) aynı baytları kendi header'ıyla gönderiyor, `x-www-form-urlencoded` ve `multipart/form-data` tipli form alanlarından kuruluyor, `text/plain` yalnız string kök alıyor. Dosya alanı `text` | `base64` | `ref` kaynaklarından tam olarak birini taşıyan bir argüman; `ref` host'un bağladığı `FileResolver` port'uyla çözülüyor ve resolver yoksa şemada hiç görünmüyor. 76 yeni fixture, toplam **367**; yeni kind açılmadı, mevcut 291 fixture bayt bayt aynı kaldı.

**Notun teşhisi yarı yanlıştı.** §3 "FastMCP daha geniş" diyordu. Kaynak okunduğunda FastMCP'nin multipart dosya parçası yalnız Python'dan çağrıldığında oluşuyor; MCP üzerinden gelen bir "dosya" dosya adı olmayan bir metin alanı, iç içe nesne ise Python repr'i olarak gidiyor. Yani FastMCP'nin genişliği tel biçiminde, ajanın erişebildiği yüzeyde değil. MCP'nin kendisinde de dosya girdisi için yayınlanmış bir standart yok (SEP-2631 taslak ve sponsorsuz); bu yüzden handle modeli bir port olarak tasarlandı, sk-mcp'nin kendi upload endpoint'i olarak değil.

**Asıl gerekçe iki bulunan hataydı.**

- `[Consumes("application/merge-patch+json")]` taşıyan bir ASP.NET endpoint'i katalogda görünüyor ve her çağrıda 415 dönüyordu, çünkü iki SDK da sabit `application/json` yazıyordu. ApiExplorer bu tipi `SupportedRequestFormats`'a bile koymuyor; kaynak endpoint'in accepts metadata'sı (`FormBindingProbeTests.P2`, `P3`).
- Nest'te her sentetik istek, body tüketildiği anda `aborted` yayıyordu: Node'un parser'ının set ettiği `IncomingMessage.complete` elle kurulan mesajda hiç set edilmiyordu. `aborted` dinleyen her handler sahte bir istemci kopması görüyordu ve multer bunu 500'e çeviriyordu (`form-body-probe` N2).

**Ölçümün düzelttiği iki sezgi.** ASP.NET form binder'ı bracket'i de okuyor (P6) — query'nin aksine; `dot` yine seçildi çünkü iki yüzeyde aynı notation. Ve urlencoded gövdenin charset parametresi zararsız değil: ASP.NET percent-escape baytlarını onunla çözüyor, `charset=us-ascii` ile `İ` → `??` (P7). Dosya adı tarafında multer düz `filename`'i latin1 okuyor (N3), `filename*` doğru okuyor (N3b); iki SDK artık aynı header'ı elle yazıyor.

**Yol üstünde kapanan açık.** .NET test projesinin fixture glob'u `packages/conformance`'a bakıyordu; corpus `packages/http/conformance`'a taşındığından beri bin'deki eski kopyalar testleri geçiriyordu ve yeni hiçbir fixture .NET runner'ına ulaşmıyordu. Glob düzeltildi, eski kopyalar her build'de temizleniyor, corpus bulunamazsa build kırılıyor ve Turbo'nun build girdisi corpus'u içeriyor.
