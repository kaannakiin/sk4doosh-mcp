...
**Durum:** Tasarım notu. §4.4 (argüman küratörlüğü) karara bağlandı ([arguman-kuratorlugu-karari.md](arguman-kuratorlugu-karari.md)), spec'lendi ([packages/spec/argument-curation.md](../../packages/spec/argument-curation.md)) ve iki SDK'da uygulandı (§7); geri kalanı karar bekliyor
**Tarih:** 17 Eylül 2026
...
| **Output schema**                 | 2xx şeması `outputSchema`; `{result}` sarma; `structured_content`; `validate_output`                                                                | `descriptor.responses` **var** (Tablo 4 response için readOnly'yi korur) ama `load_tool` sadece `{name, description, inputSchema, annotations}` döner; `responses` tool yüzeyine hiç çıkmaz    | **FastMCP.** Zor kısım bizde hazır (§4.1)                                                                                                        |
...
### 4.1 `outputSchema`'yı yüzeye çıkar
...
| ---- | ----------------------------------------------------- | ------------------------------------------------------------ | ----------------------------------------- |
| 1    | §4.1 `outputSchema` + §4.2 payload bütçesi ve timeout | metadata-contract, search-semantics, error-mapping; iki şema | metadata-extraction, error-mapping        |
...
Adım 1 ve 2 küçük spec değişikliği + fixture ile kapanır. **Adım 5 sıradan çıktı ve önce sevk edildi** — gerekçesi ve sonucu §7'de. Adım 6 ve §4.9 talep gelince.
...
Notun geri kalan maddeleri açık: §4.1 `outputSchema`, §4.2 payload bütçesi ve timeout, §4.3 parametre indeksleme, §4.5 `detail`, §4.6 `selection.rule`, §4.7 `tags`, §4.8 form/multipart body, §4.9 `deepObject` ve cookie. §5'in kopyalanmayacaklar listesi ve dört tartışma noktası aynen duruyor.
