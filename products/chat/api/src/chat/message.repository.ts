import type { MessageRole, MessageRow, TurnOutcome } from "@chat/db";
...
/**
 * Guard: `parts` is written as a json array and read back as arbitrary json. A row
 ...
 */
function asParts(value: unknown): readonly unknown[] {
...
export class MessageRepository {
   ...
   */
  async reconcileTurn(params: ReconcileParams): Promise<boolean> {
   ...
   * refusal — but the check belongs to the statement, because the next caller
   * inherits no such gate. The predicate also closes a window the caller cannot:
   * the two run in separate transactions, so a session soft deleted in between
   * would still take this write without `deleted_at`.
   */
  async settleTurn(
    userId: UserId,
    sessionId: string,
  ...
  async listMessages(
    ...
    const session = await this.db.client.chatSession.findFirst({
    ...
    const total = await this.db.client.chatMessage.count({
      where: { sessionId: session.id },
    });
    const rows = await this.db.client.chatMessage.findMany({
      where: { sessionId: session.id },
      orderBy: { seq: "desc" },
      take: limit,
    });
