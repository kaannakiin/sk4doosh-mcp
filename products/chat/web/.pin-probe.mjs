...
async function run(gip) {
  ...
  await act(async () => {
    root.render(h(Harness, { gip }));
  });

  const inputs = [...host.querySelectorAll("input[type=tel]")];
  inputs[0].focus();

  const setVal = dom.window.Object.getOwnPropertyDescriptor(
  ...
  console.log(
    `gip=${gip} -> values=[${inputs.map((i) => i.value).join(",")}] focusedIndex=${focusedIndex}`,
  );
  await act(async () => {
    root.unmount();
  });
