...
export const requiredNames = ["SKMCP_LLM_MODEL"] as const;
...
export function readLlmEnv(env: EnvRecord, cwd: string): EnvOutcome {
  ...
  const baseUrl = httpUrl(env["SKMCP_LLM_BASE_URL"]);
  const contextTokens = count(env["SKMCP_LLM_NUM_CTX"], 16_384);
  const timeoutMs = count(env["SKMCP_LLM_TIMEOUT_MS"], 300_000);
  const keepAlive = env["SKMCP_LLM_KEEP_ALIVE"] || "30m";
  if (baseUrl === undefined) {
    return {
      kind: "invalid",
      reason: "SKMCP_LLM_BASE_URL must be an http or https URL.",
    };
  }
  if (contextTokens === undefined || contextTokens < minContextTokens) {
    return {
      kind: "invalid",
      reason: `SKMCP_LLM_NUM_CTX must be an integer of at least ${minContextTokens}.`,
    };
  }
  if (timeoutMs === undefined) {
    return {
      kind: "invalid",
      reason: "SKMCP_LLM_TIMEOUT_MS must be a positive integer.",
    };
  }
  return {
    kind: "config",
