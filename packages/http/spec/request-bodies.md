...
- **`JsonPatchDocument<T>` is still reflected as an object.** Its wire form is an RFC 6902 array; that is a type-mapping gap, not a media-type one. A `[Consumes("application/json-patch+json")]` endpoint now receives the right header.
