...
- **sk-mcp offers no upload endpoint of its own.** Out-of-band upload with an `mcp-file://` handle (SEP-2631) is a draft without a sponsor, and a shape guessed today would break when a standard lands. The `ref` port is shaped to accept such a handle when one exists.
