...
- `kind: "search"` — `input`: `{tools[], query, tags?}` (each tool carrying `name` plus `route`, and optional description fields; an optional `inputSchema` is carried raw and projected to the `parameters` field by the runner, so every implementation exercises the projection; the optional `tags` list is the `search_tools` tag filter — an argument of the search, not a property of any one tool); `expected`: `{"names": [...]}` in the expected order. Rules: [search-semantics.md](search-semantics.md).
