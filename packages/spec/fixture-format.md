...
- `kind: "error-mapping"` — `input`: a `BackendResponseSpec` (`status`, `contentType?`, `headers?`, `body?`, `knownFields?`, `fieldAliases?`, `hiddenFields?`); `expected`: an `InvokeSuccess` or `MappedError` conforming to [invoke-result.schema.json](schemas/invoke-result.schema.json). Rules: [error-mapping.md](error-mapping.md).
