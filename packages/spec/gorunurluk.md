...
- **Maliyet politikası host'undur.** Probe'un ne kadar agresif koşacağı backend'e göre değişir: veritabanı yükü, rate limiter, kabul edilebilir arama gecikmesi her kurulumda farklıdır ([karar 003](../../docs/kararlar/003-istek-ustverisi.md) cetveli). SDK üç ayar sunar ve makul default'lar seçer: aday sayısı (`ProbeTopK`, sıralama sonrası ilk K), eşzamanlılık (`ProbeConcurrency`, 1 = sıralı), ve çağıran-kapsamlı sonuç önbelleği (`ProbeCacheLifetime`, sıfır = kapalı). Önbellek anahtarı **kimlik taşıyıcılarının değerlerinin özeti + tool adı**'dır: aynı taşıyıcılar aynı sentetik kimliği kurar, dolayısıyla aynı verdict'i üretir; farklı çağıran farklı anahtar alır. Taşıyıcı değerleri anahtarda düz metin tutulmaz. Önbellek ömrü aynı zamanda yetki değişikliğinin görünürlüğe yansıma gecikmesidir. Uyarı: `Identity.Project` dış istek dışında bir kaynaktan (saat, sayaç, veritabanı) değer türetiyorsa anahtar o değişkeni görmez; öyle bir projeksiyonda önbellek kapatılmalıdır.
