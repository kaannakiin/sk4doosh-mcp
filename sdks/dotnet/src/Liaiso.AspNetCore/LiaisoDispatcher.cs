...
internal sealed class LiaisoDispatchTimeout()
    : Exception("liaiso: the backend did not answer within the invoke deadline.");

/// <summary>Raised when a <c>ref</c> was not delivered; the meta-tool layer turns it into an envelope.</summary>
internal sealed class LiaisoFileRefused(string field, string reason, int limit)
    : Exception($"liaiso: file argument '{field}' was refused ({reason}).")
{
    ...
    ILogger<LiaisoDispatcher> logger)
{
        ...
        HttpMethod method, string path, HttpRequest? outerRequest, CancellationToken cancellationToken)
    {
        ...
        LiaisoProbe.MarkProbe(context);

        await RunAsync(pipeline, context);

        return new ProbeOutcome(context.Response.StatusCode, LiaisoProbe.WasShortCircuited(context));
    }
    ...
    /// <c>internal_error</c> blamed on liaiso and carrying the exception message to the agent,
    ...
    private async Task RunAsync(RequestDelegate pipeline, HttpContext context)
    {
        ...
        {
            logger.LogError(
                ex, "liaiso: an unhandled exception escaped {Method} {Path}; answered 500.",
    ...
    private RequestDelegate Pipeline() => holder.Pipeline
        ?? throw new InvalidOperationException(
            "liaiso pipeline is not captured. Call app.UseLiaisoCapture() before routing and start the host first.");
        ...
        HttpRequest? outerRequest, DispatchFiles? files, CancellationToken cancellationToken)
    {
        ...
        {
            Files.ILiaisoFileResolver resolver = services.GetService(typeof(Files.ILiaisoFileResolver))
                as Files.ILiaisoFileResolver
                ?? throw new InvalidOperationException(
                    $"liaiso: file argument '{field}' is a ref but no file resolver is registered.");
            if (files is null)
            {
                throw new InvalidOperationException(
                    $"liaiso: file argument '{field}' is a ref but the call carries no file budget.");
            }
            ...
            switch (outcome)
            {
                case Files.FileResolution.Refused refused:
                    throw new LiaisoFileRefused(field, refused.Reason switch
                    {
                ...
                default:
                    throw new InvalidOperationException("liaiso: the file resolver returned an unknown resolution.");
            }
        };
    }
