...
namespace Liaiso.AspNetCore.Visibility;
...
internal sealed class CallerVisibilityProvider
{
    ...
    private readonly ILiaisoCache cache;
    private readonly IOptions<LiaisoOptions> options;
    ...
    public CallerVisibilityProvider(
        ...
        ILiaisoCatalogChangeSource changeSource)
    {
    ...
    private async Task<string?> TryGetAsync(CacheKey key, CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogWarning(ex, "liaiso cache read failed for '{Key}'; treating as a miss.", key);
            return null;
        }
    }
    ...
    private async Task TrySetAsync(CacheKey key, string value, TimeSpan lifetime, CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogWarning(ex, "liaiso cache write failed for '{Key}'.", key);
        }
    }
