...
namespace Liaiso.AspNetCore.Tools;

[McpServerToolType]
internal sealed class LiaisoMetaTools(
    LiaisoCatalogProvider catalog,
    LiaisoDispatcher dispatcher,
    ...
    ILogger<LiaisoMetaTools> logger)
{
    ...
    /// liaiso envelope from <see cref="Respond"/>. The attribute only decorates the published schema, which
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return ErrorResult(SdkErrorCode.NotInvocable, $"Operation '{name}' cannot be invoked through liaiso; see the catalog diagnostics.");
        }
        ...
        try
        {
            ...
            catch (LiaisoDispatchTimeout)
            {
                return Respond(
                    SdkErrors.RefuseTimedOut((int)deadline.TotalMilliseconds),
                    isError: true);
            }
            catch (LiaisoFileRefused refused)
            {
                return Respond(
        ...
        catch (LiaisoArgumentException ex)
        {
            return Respond(
    ...
    /// agent without passing the payload budget. <see cref="LiaisoBudgetTool"/> backs it up for any
    ...
    {
        string json = JsonSerializer.Serialize(payload, LiaisoJson.Wire);
        ...
        SdkError refusal = SdkErrors.RefuseOversize(new OversizeResponse(
            bytes,
            limit,
            SdkErrors.Describe(JsonSerializer.SerializeToNode(summaryOf ?? payload, LiaisoJson.Wire)),
            narrowing));
        return Wire(JsonSerializer.Serialize(refusal, LiaisoJson.Wire), isError: true);
    }
