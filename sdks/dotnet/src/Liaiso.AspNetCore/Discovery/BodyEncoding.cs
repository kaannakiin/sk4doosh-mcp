    ...
    private static bool Settle(string chosen, string where, List<CatalogDiagnostic> diagnostics, out string? contentType)
    {
        ...
        if (!IsWritable(chosen))
        {
            diagnostics.Add(new CatalogDiagnostic(
                ...
                $"{where} takes a {chosen} body, which liaiso has no writer for; endpoint skipped."));
            return false;
        }
        return true;
    }
        ...
        List<CatalogDiagnostic> diagnostics)
    {
        if (metadata.OfType<IAntiforgeryMetadata>().LastOrDefault() is { RequiresValidation: true })
        {
            diagnostics.Add(new CatalogDiagnostic(
                ...
                $"{where} requires antiforgery validation, which a synthetic request cannot pass and liaiso does not bypass. Opt the endpoint out with .DisableAntiforgery() if it is not reached from a browser; endpoint skipped."));
            return null;
        }
