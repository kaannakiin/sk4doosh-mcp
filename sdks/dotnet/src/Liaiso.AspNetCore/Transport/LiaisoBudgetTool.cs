...
/// <summary>
/// The backstop for the payload budget. <c>LiaisoMetaTools.Respond</c> is the primary gate, but
...
/// </remarks>
internal sealed class LiaisoBudgetTool(McpServerTool inner, IOptions<LiaisoOptions> options)
    : DelegatingMcpServerTool(inner)
{
        ...
        catch (Exception ex)
        {
            ...
            return Refuse(SdkErrors.Create(
                ...
                forwardable is null
                    ...
                    : $"The operation failed inside the liaiso layer: {forwardable}"));
        }
    ...
    {
        IsError = true,
        Content = [new TextContentBlock { Text = JsonSerializer.Serialize(error, LiaisoJson.Wire) }],
    };
