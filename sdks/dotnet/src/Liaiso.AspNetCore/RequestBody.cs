...
internal static partial class RequestBodyEncoder
{
    ...
    private static LiaisoArgumentException InvalidFile(string field, string detail) =>
        new(LiaisoArgumentException.InvalidFileArgument, $"File argument '{field}' {detail}");
    ...
    private static IEnumerable<JsonElement> ItemsOf(JsonElement value, bool isArray, string name)
    {
        ...
        if (value.ValueKind != JsonValueKind.Array)
        {
            throw new LiaisoArgumentException(
                LiaisoArgumentException.InvalidType, $"Body argument '{name}' must be an array.");
        }
                ...
                default:
                    ...
                    foreach (JsonElement element in ItemsOf(item, field.IsArray, field.Name))
                    {
                        parts.Add(new FieldPart(
                            ...
                            RequestComposer.FormatScalar(element, slot, LiaisoArgumentException.InvalidType)));
                    }
                    break;
            }
        }
        ...
        List<ComposedPart> parts, FormBinding form, FormField field, JsonElement item, Func<string, string> encodeName)
    {
        ...
        {
            throw new LiaisoArgumentException(
                LiaisoArgumentException.InvalidType, $"Body argument '{field.Name}' must be an object.");
        }
        ...
        foreach (ObjectMember member in members)
        {
            ...
            {
                parts.Add(new FieldPart(key, RequestComposer.FormatScalar(element, slot, LiaisoArgumentException.InvalidType)));
            }
        }
    }
        ...
        IReadOnlyList<ComposedPart> parts, ComposeLimits? limits, IReadOnlySet<FileSource> sources)
    {
        ...
        if (total > limit)
        {
            string advice = sources.Contains(FileSource.Ref)
                ...
                : "Send a smaller file.";
            throw new LiaisoArgumentException(
                LiaisoArgumentException.FileTooLarge,
    ...
    public static ComposedBody? Encode(RequestTemplate template, JsonElement? value, ComposeLimits? limits)
    {
        ...
        if (contentType == MediaTypes.Text)
        {
            ...
            {
                throw new LiaisoArgumentException(
                    LiaisoArgumentException.InvalidType, $"Argument '{argument}' must be of type string.");
            }
        ...
        {
            throw new LiaisoArgumentException(
                LiaisoArgumentException.InvalidType, $"Argument '{argument}' must be an object.");
        }
