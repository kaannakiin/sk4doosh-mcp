...
internal sealed record TestApp(WebApplication App, HttpClient Client, LiaisoDispatcher Dispatcher) : IAsyncDisposable
{
...
{
    public const string SigningKey = "liaiso-tests-signing-key-at-least-32-bytes-long!!";
    ...
    public static async Task<TestApp> GateAppAsync(
        Action<LiaisoOptions>? configure, Func<HttpContext, bool> gate)
    {
        WebApplicationBuilder builder = WebApplication.CreateBuilder();
        builder.WebHost.UseTestServer();
        builder.Logging.ClearProviders();
...
public class IdentityCarrierTests
{
    ...
    {
        Action<LiaisoOptions> configure = forwardCsrfHeader
