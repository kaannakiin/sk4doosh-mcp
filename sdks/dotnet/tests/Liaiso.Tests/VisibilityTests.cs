...
public sealed class VisibilityTests
{
    ...
    {
        await using Harness host = await HostAsync();
        LiaisoMetaTools bob = host.ToolsFor(Mint("bob"));
    ...
    public async Task V8_VisibilityAndInvoke_ShareIdentityComposition()
    {
        ...
        LiaisoMetaTools alice = host.ToolsFor(Mint("alice", ordersRead: true));
    ...
    {
        await using Harness host = await HostAsync();
        LiaisoMetaTools alice = host.ToolsFor(Mint("alice", ordersRead: true, role: "admin"));
    ...
    public async Task V17_ProbeFlag_CannotBeSetByRealRequests()
    {
        ...
        request.Headers.TryAddWithoutValidation("liaiso.probe", "true");
        request.Headers.TryAddWithoutValidation("X-Liaiso-Probe", "1");
        request.Headers.TryAddWithoutValidation("liaiso.synthetic", "true");
        request.Headers.TryAddWithoutValidation("User-Agent", "liaiso/9.9.9");
    ...
    private static async Task<HashSet<string>> SearchOverTransportAsync(Harness host, string token)
    {
        ...
        CallToolResult result = await client.CallToolAsync(
            "search_tools",
            new Dictionary<string, object?> { ["query"] = "", ["limit"] = LiaisoMetaTools.MaxLimit },
