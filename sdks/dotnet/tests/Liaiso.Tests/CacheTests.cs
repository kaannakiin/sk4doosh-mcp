...
using static Liaiso.Tests.VisibilityHost;
...
internal sealed class RecordingCache : ILiaisoCache
{
    ...
    {
        foreach (string existing in _values.Keys.Where(k => k.StartsWith($"liaiso:v1:{scopeKey}:", StringComparison.Ordinal)))
        {
...
public sealed class CacheTests
{
    ...
    {
        IOptions<LiaisoOptions> options = Options.Create(new LiaisoOptions());
    ...
    public async Task K4_Facts_ComputedOnceWithinLifetime()
    {
        ...
        await using Harness host = await HostAsync(beforeLiaiso: services =>
        ...
        LiaisoMetaTools tools = host.ToolsFor(Mint("alice", ordersRead: true));
    ...
    public async Task K5_SingleFlight_ParallelSearches_ProbeOnce()
    {
        ...
        LiaisoMetaTools tools = host.ToolsFor(Mint("alice"));
    ...
    public async Task K5_SingleFlight_ParallelHosts_HaveIndependentProbeCounts()
    {
        ...
        LiaisoMetaTools firstTools = first.ToolsFor(Mint("alice"));
        LiaisoMetaTools secondTools = second.ToolsFor(Mint("alice"));
    ...
    public async Task K7_MaxCallers_EvictsLeastRecentlyUsedScope()
    {
        ...
        LiaisoMetaTools alice = host.ToolsFor(Mint("alice"));
        Interlocked.Exchange(ref host.Probes.UndeclaredHits, 0);
    ...
    public async Task K9_InvalidateCaller_AffectsOnlyThatCaller()
    {
        ...
        LiaisoMetaTools aliceTools = host.ToolsFor(aliceToken);
        LiaisoMetaTools bobTools = host.ToolsFor(bobToken);
        ...
        ILiaisoCacheInvalidator invalidator = host.App.Services.GetRequiredService<ILiaisoCacheInvalidator>();
    ...
    {
        await using Harness host = await ProbeHostAsync(beforeLiaiso: services =>
            services.AddSingleton<ICallerScopeResolver>(sp =>
                new TaggingScopeResolver(new CarrierHashCallerScopeResolver(sp.GetRequiredService<IOptions<LiaisoOptions>>()))));
        DynamicGrantAttribute.Grants.Clear();
        DynamicGrantAttribute.Grants.Add("alice");

        LiaisoMetaTools session1 = host.ToolsFor(Mint("alice", role: "session1"), ("X-User", "alice"));
        LiaisoMetaTools session2 = host.ToolsFor(Mint("alice", role: "session2"), ("X-User", "alice"));
    ...
    public async Task K11_InvalidateAll_AffectsEveryone()
    {
        ...
        LiaisoMetaTools aliceTools = host.ToolsFor(Mint("alice"));
        LiaisoMetaTools bobTools = host.ToolsFor(Mint("bob"));
        Assert.Contains("vis_dynamic", (await SearchAsync(aliceTools)).Names);
        Assert.Contains("vis_dynamic", (await SearchAsync(bobTools)).Names);
    ...
    public async Task K12_DoneCriterion_GrantRevokeInvalidateWithoutRestart()
    {
        ...
        LiaisoMetaTools alice = host.ToolsFor(Mint("alice"), ("X-User", "alice"));
        LiaisoMetaTools bob = host.ToolsFor(Mint("bob"), ("X-User", "bob"));
    ...
    [Fact]
    public async Task K13_HostCache_RegisteredBeforeAddLiaiso_IsUsed()
    {
        RecordingCache recording = new();
        await using Harness host = await HostAsync(beforeLiaiso: services =>
            services.AddSingleton<ILiaisoCache>(recording));
    ...
    [Fact]
    public async Task K14_HostCache_RegisteredAfterAddLiaiso_IsUsed()
    {
        RecordingCache recording = new();
        await using Harness host = await HostAsync(afterLiaiso: services =>
            services.AddSingleton<ILiaisoCache>(recording));
        await SearchAsync(host.ToolsFor(Mint("alice", ordersRead: true)));
    ...
    public async Task K15_HostResolver_CollapsesTwoTokensIntoOneScope()
    {
        ...
        LiaisoMetaTools first = host.ToolsFor(Mint("alice", ordersRead: true), ("X-Tenant", "acme"));
        LiaisoMetaTools second = host.ToolsFor(Mint("bob"), ("X-Tenant", "acme"));
    ...
    public async Task K16_CatalogReload_ClearsCacheAndDisabledSet_SignalsChangeToken()
    {
        ...
        LiaisoCatalogProvider catalog = host.App.Services.GetRequiredService<LiaisoCatalogProvider>();
    ...
    [Fact]
    public void K19_AddLiaiso_CalledTwice_RegistersOnce()
    {
        ServiceCollection services = new();
        services.AddLiaiso();
        services.AddLiaiso();

        Assert.Single(services, d => d.ServiceType == typeof(LiaisoCatalogProvider));
