...
public sealed class ResponseBudgetTests
{
    ...
    private sealed record BudgetHarness(WebApplication App, LiaisoMetaTools Tools, LiaisoOptions Options) : IAsyncDisposable
    {
    ...
    private static async Task<BudgetHarness> HostAsync(Action<LiaisoOptions>? configure = null)
    {
        ...
        LiaisoMetaTools tools = new(
            app.Services.GetRequiredService<LiaisoCatalogProvider>(),
            app.Services.GetRequiredService<LiaisoDispatcher>(),
        ...
        return new BudgetHarness(app, tools, app.Services.GetRequiredService<IOptions<LiaisoOptions>>().Value);
    }

    private static async Task<(SdkError? Error, JsonElement Raw, bool IsError)> InvokeAsync(
        LiaisoMetaTools tools, string name, object arguments)
    {
        ...
        SdkError? error = raw.TryGetProperty("error", out _) && !raw.TryGetProperty("status", out _)
            ? JsonSerializer.Deserialize<SdkError>(text, LiaisoJson.Wire)
            : null;
