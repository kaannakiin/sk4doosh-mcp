...
public sealed class VisibilityController : ControllerBase
{
    ...
    {
        probe = HttpContext.IsLiaisoProbe(),
        synthetic = HttpContext.IsLiaisoRequest(),
...
internal sealed record Harness(WebApplication App, LiaisoDispatcher Dispatcher, ProbeCounter Probes) : IAsyncDisposable
{
    public LiaisoMetaTools ToolsFor(string? token, params (string Name, string Value)[] headers)
    {
        ...
        return new LiaisoMetaTools(
            App.Services.GetRequiredService<LiaisoCatalogProvider>(),
            Dispatcher,
            ...
            App.Services.GetRequiredService<IOptions<LiaisoOptions>>(),
            new FixedContext(outer),
            App.Services.GetRequiredService<ILogger<LiaisoMetaTools>>());
    }
...
internal static class VisibilityHost
{
    ...
    public static async Task<Harness> HostAsync(
        ...
        Action<IServiceCollection>? beforeLiaiso = null,
        Action<IServiceCollection>? afterLiaiso = null)
    {
        ...
        beforeLiaiso?.Invoke(builder.Services);
        builder.Services.AddLiaiso(configure);
        afterLiaiso?.Invoke(builder.Services);
        ...
        return new Harness(app, app.Services.GetRequiredService<LiaisoDispatcher>(), probes);
    }
    ...
    public static Task<Harness> ProbeHostAsync(
        ...
        HostAsync(
            ...
            beforeLiaiso: beforeLiaiso,
            afterLiaiso: afterLiaiso);

    public static async Task<(HashSet<string> Names, HashSet<string> Uncertain, int Total, string Raw)> SearchAsync(
        LiaisoMetaTools tools, string query = "")
    {
        string raw = TextOf(await tools.SearchTools(query, LiaisoMetaTools.MaxLimit));
    ...
    public static async Task<int> InvokeStatusAsync(LiaisoMetaTools tools, string name, object arguments)
    {
