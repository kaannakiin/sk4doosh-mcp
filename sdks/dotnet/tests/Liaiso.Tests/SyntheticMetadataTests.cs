...
{
    public static async Task<TestApp> StartAsync(Action<LiaisoOptions>? configure = null)
    {
        ...
        app.MapGet("/meta", (HttpContext ctx) => string.Join('|',
            ...
            ctx.IsLiaisoRequest() ? "synthetic" : "outer"));
...
public class SyntheticMetadataTests
{
    ...
    {
        await using TestApp app = await MetaHost.StartAsync();
        Assert.StartsWith("liaiso/", (await Meta(app, null)).Body.Split('|')[3]);
    ...
    public async Task M9_SyntheticMarker_CannotBeSetByOuterRequest()
    {
        ...
        spoof.Headers.TryAddWithoutValidation("liaiso.synthetic", "true");
        spoof.Headers.TryAddWithoutValidation("User-Agent", "liaiso/9.9.9");
