...
internal static class EchoHost
{
    public static async Task<(TestApp App, SentinelFlag Sentinel)> StartAsync()
    {
        ...
        return (new TestApp(app, app.GetTestClient(), app.Services.GetRequiredService<LiaisoDispatcher>()),
...
public class ArgumentMappingTests
{
    ...
    public void A3_HeaderControlCharacters_Rejected()
    {
        ...
        LiaisoArgumentException ex = Assert.Throws<LiaisoArgumentException>(() =>
            RequestComposer.Compose(template, Args(new Dictionary<string, string> { ["X-Data"] = "x\r\nEvil: 1" })));
        Assert.Equal(LiaisoArgumentException.HeaderInjection, ex.Code);
    }
    ...
    public async Task A4_PathTypeGate_PreventsOpaque404()
    {
        ...
        Assert.Equal(LiaisoArgumentException.InvalidPathType, ex.Code);
    ...
    public void A5_UnknownArgument_ErrorListsAllowed()
    {
        ...
        Assert.Equal(LiaisoArgumentException.UnknownArgument, ex.Code);
    ...
    public async Task A6_AbsentOmitted_NullRejected()
    {
        ...
        Assert.Equal(LiaisoArgumentException.NullNotAllowed, ex.Code);
    }
    ...
    {
        Assert.Throws<LiaisoTemplateException>(() => RequestTemplate.Create(
    ...
    public void A13_ObjectBindingGuards()
    {
        ...
        Assert.Contains("only a non-array query parameter", Assert.Throws<LiaisoTemplateException>(
            () => Build(new ParameterBinding(
                "filter", ParameterLocation.Header, ParameterKind.String, Members: one))).Message,
            StringComparison.Ordinal);

        Assert.Contains("cannot be hidden or filled", Assert.Throws<LiaisoTemplateException>(
            () => Build(new ParameterBinding(
        ...
        Assert.Contains("declares no members", Assert.Throws<LiaisoTemplateException>(
            () => Build(new ParameterBinding(
                "filter", ParameterLocation.Query, ParameterKind.String, Members: []))).Message,
            StringComparison.Ordinal);

        Assert.Contains("two members named", Assert.Throws<LiaisoTemplateException>(
            () => Build(new ParameterBinding(
        ...
        {
            LiaisoTemplateException structural = Assert.Throws<LiaisoTemplateException>(
                () => Build(new ParameterBinding(
                    "filter", ParameterLocation.Query, ParameterKind.String,
                    Members: [new ObjectMember(name, ParameterKind.String)])));
            Assert.Equal(LiaisoTemplateException.UnsupportedObjectNesting, structural.Code);
    ...
    public void A11_ArraySeparatorGuards()
    {
        ...
        {
            LiaisoTemplateException exploded = Assert.Throws<LiaisoTemplateException>(
                () => RequestTemplate.ArraySeparatorFor(style, true, "tag"));
            Assert.Equal(LiaisoTemplateException.UnsupportedArrayStyle, exploded.Code);
        ...
        {
            LiaisoTemplateException deep = Assert.Throws<LiaisoTemplateException>(
                () => RequestTemplate.ArraySeparatorFor("deepObject", explode, "tag"));
            Assert.Equal(LiaisoTemplateException.UnsupportedArrayStyle, deep.Code);
