...
public sealed class UnhandledExceptionHostTests : InvokeHost
{
    ...
    public async Task UX1_AHandlerExceptionIsABackendErrorWithItsMessageWithheld(string name)
    {
        ...
        Assert.DoesNotContain("sk-mcp layer", raw.GetRawText(), StringComparison.Ordinal);
    }
    ...
    {
        ProbeOutcome outcome = await Services.GetRequiredService<SkMcpDispatcher>().ProbeAsync(
